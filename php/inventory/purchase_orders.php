...
if($_POST){
    if(isset($_POST['create_order'])){
        ...
        error_log("Attempting to create purchase order with:");
        error_log("Supplier ID: " . $purchaseOrder->supplier_id);
        error_log("Order Date: " . $purchaseOrder->order_date);
        error_log("Expected Date: " . $purchaseOrder->expected_date);
        error_log("Status: " . $purchaseOrder->status);
            ...
            try {
                if($purchaseOrder->create()){
                    ...
                    echo "<div class='alert alert-success'>Purchase order created successfully! 
                          <a href='purchase_order_items.php?order_id=$order_id' style='margin-left: 10px;'></a></div>";
                } else {
